...
defmodule Storybook.Examples.FormDemo do
  ...
  import MoonStorybookWeb.CoreComponents, only: [icon: 1]
  ...
  def doc do
    "An example of convert React template for Shadcn ui to heex template using MoonUI."
  end
  ...
  def handle_event("create_item", %{"item" => item_params} = params, socket) do
    case %Item{} |> Item.changeset(item_params) |> Ecto.Changeset.apply_action(:insert) do
      {:error, changeset} ->
        {:noreply,
         assign(socket, form: to_form(changeset), output: inspect(params, pretty: true, width: 0))}
